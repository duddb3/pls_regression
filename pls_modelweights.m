function [b_mean_coeff,b_Z_coeff,b_P_coeff,b_mean_intercept] = pls_modelweights(X,Y,ncomp)
    ...
    r_bs_int = squeeze(r_bs_b(:,1,:));
    r_bs_b = r_bs_b(:,2:end,:);

    b_mean_coeff = squeeze(mean(r_bs_b,'omitnan'));
    b_mean_intercept = squeeze(mean(r_bs_int,'omitnan'));
